...
export class jsonToArrConvertService {
            ...
            res.forEach((object: any)=>{
                ...
                if (!data.has(object.src_office_id)) {
                    ...
                    Object.keys(object).forEach((key)=>{
                        ...
                        if (key !== 'src_office_id' && key!=='dt_date' && key!=="office_name") 
                            chartDatasets.push({
                                label: key,
                    ...
                    data.set(object.src_office_id,{
                        title: object.office_name || `office id: ${object.src_office_id.toString()}`,
                    ...
                    if (graphData) {
                        Object.keys(object).forEach((key)=>{
                            if (key == 'dt_date')
                                ...
                                if (dataSetArr)
                                    dataSetArr.data.push(object[key])
                            }
                        })
    ...
    calcSumData():void {
        ...
        const allDateArr: Set<string> = new Set(...this.chartsData.map((el)=>el.label));
        ...
        for (let date of allDateArr.values()) {
            ...
            Object.keys(sumsCalculateObj).forEach((key)=>{
                ...
                if (dataSetArr)
                    dataSetArr.data.push(sumsCalculateObj[key])
            })
        }
    }
}